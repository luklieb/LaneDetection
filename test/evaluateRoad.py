...
class dataStructure: 
    ...
    gt_end = '.png'
    prob_end = '.png'
    eval_propertyList = ['MaxF', 'AvgPrec', 'PRE_wp', 'REC_wp', 'TPR_wp', 'FPR_wp', 'FNR_wp', 'TP_wp', 'FP_wp', 'FN_wp', 'TN_wp']
...
def main(result_dir, train_dir, debug = False):
    ...
    for cat in dataStructure.cats:
	    ...
	    prob_eval_scores.append(pxEval_maximizeFMeasure(posNum, negNum, FN, FP, thresh = thresh))
            factor = 100
            for property in dataStructure.eval_propertyList:
                print '%4.2f ' %(prob_eval_scores[-1][property]*factor),
            print
